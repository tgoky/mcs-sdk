...
//
// Unlike the other four bridges forms, saving here also turns Pin-Down on
// and runs it immediately — onSaved hands the caller the raw save
// response (with `runId` when one was dispatched) so each context can
// decide what "saved" should do: the standalone page navigates to the
// live run; WorkersPanel does the same, since watching a real run beats
// staring at a static card regardless of where Configure was opened from.

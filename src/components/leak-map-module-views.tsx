...
// ---------------------------------------------------------------------------
// Action & Diagnostic Summaries for Leak Map
// ---------------------------------------------------------------------------
...
function buildRunSections(
  ...
  if (isRunning) {
    ...
    runControl.push({
      key: "retrigger",
      icon: Waves,
      label: "Generate fresh Leak Map",
      disabled: !run.engagementId,
      onSelect: () =>
        run.engagementId &&
        dispatch("retrigger", () => triggerSkillRun(run.engagementId as string, "leak-map"), () => { onDone(); closePanel(); }),
    });
  }
...
export function LeakMapModuleView({
  ...
  const refresh = useCallback(async (signal?: AbortSignal) => {
    try {
      const res = await fetch("/api/skill-runs/recent?skill=leak-map&limit=100", { cache: "no-store", signal });
      if (!res.ok) return;
                    ...
                    {expanded &&
                      group.items.slice(1).map((subRun) => (
                        <tr
                          ...
                          className="group bg-zinc-950/40 hover:bg-zinc-800/30 transition-colors cursor-pointer border-l-2 border-l-sky-400/50"
                        >

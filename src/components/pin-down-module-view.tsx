...
function actionSummary(run: SkillRun): string {
  ...
  if (s === "failed" || s === "error") {
    ...
    return "Onboarding setup failed — click to view diagnostic log";
  }
  if (s === "timed_out") return "Timed out — exceeded max execution runtime";
  if (s === "cancelled") return "Cancelled by operator";
  return "Client account set up & confirmation page deployed";
}
...
function buildRunSections(
  ...
  if (isRunning) {
    runControl.push({
      key: "cancel",
      icon: Ban,
      label: "Cancel this onboarding run",
      tone: "danger",
    ...
    runControl.push({
      key: "retrigger",
      icon: RotateCcw,
      label: "Re-run Pin-Down Setup",
      disabled: !run.engagementId,
      onSelect: () =>
        run.engagementId &&
        dispatch("retrigger", () => triggerSkillRun(run.engagementId as string, "pin-down"), () => { onDone(); closePanel(); }),
    });
  }
                    ...
                    {expanded &&
                      group.items.slice(1).map((subRun) => (
                        <tr
                          ...
                          className="group bg-zinc-950/40 hover:bg-zinc-800/30 transition-colors cursor-pointer border-l-2 border-l-amber-400/50"
                        >

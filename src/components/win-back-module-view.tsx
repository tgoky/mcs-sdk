...
// ---------------------------------------------------------------------------
// Action & Diagnostic Summaries for Win-Back
// ---------------------------------------------------------------------------
...
function buildRunSections(
  ...
  if (isRunning) {
    runControl.push({
      key: "cancel",
      icon: Ban,
      label: "Cancel this recovery run",
      tone: "danger",
    ...
    runControl.push({
      key: "retrigger",
      icon: RotateCcw,
      label: "Re-run Win-Back Recovery",
      disabled: !run.engagementId,
      onSelect: () =>
        run.engagementId &&
        dispatch("retrigger", () => triggerSkillRun(run.engagementId as string, "win-back"), () => { onDone(); closePanel(); }),
    });
  }
...
export function WinBackModuleView({
  ...
  const refresh = useCallback(async (signal?: AbortSignal) => {
    try {
      const res = await fetch("/api/skill-runs/recent?skill=win-back&limit=100", { cache: "no-store", signal });
      if (!res.ok) return;
  ...
  return (
    <div className="space-y-3 font-sans antialiased text-zinc-100">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
        ...
        {/* Live Controls + List/Board Switcher */}
                ...
                return (
                  <Fragment key={group.signature}>
                    ...
                    >
                      ...
                      {/* Skill Member Badge: WB in Rose/Coral */}
                      <td className="px-4 py-3.5 text-center">
                        <span
                          className="inline-flex items-center justify-center h-6 w-6 rounded-full text-[10px] font-bold text-zinc-950 bg-[#fb7185] shadow-xs"
                          title="Win-Back Module"
                        >
                          WB
                        </span>
                      </td>
                    ...
                    {expanded &&
                      group.items.slice(1).map((subRun) => (
                        <tr
                          ...
                          className="group bg-zinc-950/40 hover:bg-zinc-800/30 transition-colors cursor-pointer border-l-2 border-l-rose-400/50"
                        >
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="inline-flex items-center justify-center h-6 w-6 rounded-full text-[10px] font-bold text-zinc-950 bg-[#fb7185] shrink-0">
                            WB
                          </span>
                          <div className="min-w-0">

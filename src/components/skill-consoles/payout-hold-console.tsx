  ...
  return (
    <div className="space-y-4">
      <button
        ...
        onClick={assemble}
        disabled={loading}
        className="inline-flex items-center gap-1.5 rounded-lg bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-4 py-2 text-sm font-bold disabled:opacity-50"
      >
        ...
        <div className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-3">
            <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 p-3">
              <h3 className="text-xs font-mono uppercase tracking-wider text-zinc-500 mb-1">Payout status</h3>
              <p className="text-[15px] font-semibold">{packet.identityProfile?.payout_status ?? packet.accountHealth?.status ?? "unknown"}</p>
              {packet.accountHealth?.status_reason && <p className="text-sm text-zinc-500 mt-1">{packet.accountHealth.status_reason}</p>}
            </div>
            <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 p-3">
              <h3 className="text-xs font-mono uppercase tracking-wider text-zinc-500 mb-1">90-day dispute rate</h3>
              <p className="text-[15px] font-semibold">{packet.chargebackRatio90d !== null ? `${(packet.chargebackRatio90d * 100).toFixed(1)}%` : "unavailable"}</p>
            </div>
          </div>
          ...
          {(packet.accountHealth?.required_actions?.length ?? 0) > 0 && (
            <div className="rounded-lg border border-amber-200 dark:border-amber-900/60 bg-amber-50/60 dark:bg-amber-950/20 p-3">
              <h3 className="text-sm font-bold mb-1">Whop&apos;s required actions</h3>
          ...
          <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 p-3">
            <h3 className="text-sm font-bold mb-1">Drafted escalation (send this yourself)</h3>
            <pre className="text-sm whitespace-pre-wrap font-sans text-zinc-700 dark:text-zinc-300">{packet.draftedEscalation}</pre>
          </div>

          <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 p-3">
            <h3 className="text-sm font-bold mb-1">Follow-up checklist</h3>
          ...
          {packet.gatheredManually.length > 0 && (
            <p className="text-sm text-zinc-500 dark:text-zinc-400">Gather manually: {packet.gatheredManually.join(", ")}</p>
          )}
        </div>
      )}
    </div>

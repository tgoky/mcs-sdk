    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-[15px] font-mono"
        />
        <button type="button" onClick={assemble} disabled={loading || !disputeId.trim()} className="rounded-lg bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-4 py-2 text-sm font-bold disabled:opacity-50">
      ...
      {assembly?.status === "window_closed" && (
        <div className="flex items-start gap-2 rounded-lg border border-rose-200 dark:border-rose-900/70 bg-rose-50 dark:bg-rose-950/30 p-3 text-sm text-rose-700 dark:text-rose-300">
      ...
      {assembly?.status === "assembled" && (
        <div className="space-y-3">
          ...
          <div>
            <label className="text-xs font-mono uppercase tracking-wider text-zinc-500">Narrative (notes field). Review before submitting</label>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={8} className="mt-1 w-full rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-sm font-mono" />
          </div>

          {assembly.gatheredManually.length > 0 && (
            <p className="text-sm text-amber-600 dark:text-amber-400">Attach manually: {assembly.gatheredManually.join(", ")}</p>
          )}

...
interface SecondarySidebarProps {
  ...
  skills: ReactNode;
  meetings: ReactNode;
  // Optional client data array if passed directly to the sidebar
  clients?: ClientItem[];
...
export function SecondarySidebar({
  ...
  // Primary navigation items for the "Work" section
  const workNavItems = [
    {
      href: "/dashboard",
      label: "Home",
    ...
    },
    {
      ...
      label: "Executions",
      icon: Activity, // Replaced Zap icon with Activity
    },
    {
  ...
  return (
    <aside className="w-60 bg-[#1e1f21] dark:bg-[#1e1f21] border-r border-zinc-800 flex flex-col h-full shrink-0 select-none py-3 px-2 overflow-y-auto font-sans antialiased text-zinc-300">
      ...
      {section === "work" ? (
        ...
        /* OTHER SECTION SLOTS (Engagements, Analytics, Strategy, Meetings) */
      ...
      {/* ----------------------------------------------------------------- */}
      {/* SKILLS SLOT (Rendered directly underneath Clients / Work Slot)    */}
      {/* ----------------------------------------------------------------- */}
      {section !== "skills" && (

...
import { WhopAgentSkillBadge } from "@/components/whop-agent-skill-badge";
  ...
  if (isRepSkillId(skill) || isColdOpenSkillId(skill) || isWhopAgentSkillId(skill)) {
    return (
      <div className="relative shrink-0" style={{ width: size, height: size }}>
        ...
        ) : isColdOpenSkillId(skill) ? (
          <ColdOpenSkillBadge skill={skill} size={size} />
        ) : (
          <WhopAgentSkillBadge skill={skill} size={size} />
        )}
        {Boolean(count) && (
          <span

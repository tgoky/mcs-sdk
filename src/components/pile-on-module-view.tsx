...
import { Activity, Building2, CheckCircle2, ChevronRight, Zap } from "lucide-react";
...
export function PileOnModuleView({
  ...
  const totalDispatches = summaries.reduce((a, b) => a + b.totalRuns, 0);
  ...
  return (
    <div className="space-y-5 font-sans">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 font-sans">
        <KpiCard label="Total Dispatches" value={totalDispatches} note="Lifetime leads enrolled" icon={Zap} tone="neutral" />
        <KpiCard label="Healthy Accounts" value={healthyCount} note="Dispatches active" icon={CheckCircle2} tone="success" />
        <KpiCard label="Latency Ceiling" value="<6.0s" note="Hybrid budget target" icon={Activity} tone="info" />
        <KpiCard label="Configured Accounts" value={summaries.length} note="Active engagements" icon={Building2} tone="neutral" />
      </div>
          ...
          <table className="w-full text-left text-xs font-sans">
            <thead>
              <tr className="border-b border-zinc-800 text-[10px] uppercase text-zinc-500 bg-zinc-900/50 font-sans">
                ...
                <th className="px-4 py-3 font-semibold">Total Dispatches</th>
            ...
            <tbody>
              {filtered.map((c) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-mono text-zinc-400">{c.totalRuns} leads</td>
                  <td className="px-4 py-3 text-right">
                    ...
                    >
                      Open Stream <ChevronRight size={12} />
                    </Link>
                  </td>
                </tr>
              ))}

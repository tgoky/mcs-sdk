...
export function ApprovalModeToggle({
  ...
  if (requireApproval) {
    return (
      <div className="flex items-center gap-2">
        <span className="inline-flex items-center gap-1 text-[11px] font-mono font-bold text-gold-hover dark:text-gold bg-gold/10 border border-gold/25 px-2 py-1 rounded">
          <ShieldCheck className="w-3 h-3" /> Co-Pilot — confirmation page needs your approval to publish
        </span>
        <button
          onClick={() => setConfirming(true)}
          disabled={busy}
          className="text-[11px] font-mono font-bold px-2 py-1 rounded border border-zinc-300 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:border-zinc-400 dark:hover:border-zinc-600 hover:text-zinc-900 dark:hover:text-zinc-200 disabled:opacity-50 transition-all cursor-pointer"
        >
          Switch to Autopilot
        </button>
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="inline-flex items-center gap-1 text-[11px] font-mono font-bold text-zinc-600 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-900/60 px-2 py-1 rounded">
        <Zap className="w-3 h-3" /> Autopilot — confirmation page publishes automatically
      </span>
      <button
        onClick={() => setMode(true)}
        disabled={busy}
        className="text-[11px] font-mono font-bold px-2 py-1 rounded border border-zinc-300 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:border-gold/40 hover:text-gold-hover dark:hover:text-gold transition-all cursor-pointer disabled:opacity-50"
      >

  ...
  if (!pack || pack.briefs.length === 0) {
    return (
      <div className="bg-transparent border border-zinc-200/60 dark:border-zinc-800/60 rounded-2xl p-5 font-sans">
        <h3 className="text-[15px] font-bold text-zinc-900 dark:text-white font-sans">Ad Creative Briefs</h3>
        <p className="text-[13px] text-zinc-500 dark:text-zinc-400 font-sans mt-1">
    ...
    <div className="bg-transparent border border-zinc-200/60 dark:border-zinc-800/60 rounded-2xl overflow-hidden font-sans">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-200 dark:border-zinc-800 px-5 py-3">
        ...
        {pack.objectionsLastRegeneratedAt && (
          <span className="flex items-center gap-1 text-xs font-mono font-semibold text-amber-600 dark:text-amber-400 shrink-0">
          ...
          return (
            <div key={b.id} className="bg-transparent border border-zinc-200/60 dark:border-zinc-800/60 rounded-xl overflow-hidden">
              <div className="px-3.5 pt-3 pb-2.5 font-sans">
                <span className="inline-flex items-center gap-1 rounded-full border border-zinc-200 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800/60 px-2 py-0.5 text-[11px] font-mono font-semibold uppercase text-zinc-600 dark:text-zinc-400">
                  <Megaphone size={9} /> {adCreativePillarLabel(b.pillar)}
                </span>
                <p className="mt-2 text-[15px] font-bold text-zinc-900 dark:text-white font-sans leading-snug">{b.hook}</p>
                <p className="mt-1 text-[13px] text-zinc-600 dark:text-zinc-400 font-sans leading-relaxed">{b.angle}</p>
              </div>
              ...
              >
                <span className="text-[13px] font-mono text-zinc-500">{expanded ? "Hide details" : "Talking points, format & CTA"}</span>
              ...
              {expanded && (
                <div className="border-t border-zinc-200 dark:border-zinc-800/60 px-3.5 py-3 space-y-2.5 bg-transparent font-sans">
                  <div>
                    <span className="text-[11px] font-mono font-bold uppercase text-zinc-500">Talking points</span>
                    <ul className="mt-1 space-y-1">
                      {b.talkingPoints.map((tp, i) => (
                        <li key={i} className="text-[13px] text-zinc-800 dark:text-zinc-300 font-sans leading-snug flex gap-1.5">
                          <span className="text-zinc-400 dark:text-zinc-600 font-mono text-xs shrink-0">{i + 1}.</span>
                          <span>{tp}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="bg-transparent border border-zinc-200/60 dark:border-zinc-800/60 rounded-lg px-2.5 py-1.5 font-sans">
                    <span className="text-[11px] font-mono uppercase text-zinc-500 block">CTA</span>
                    <span className="text-sm font-medium text-zinc-900 dark:text-white font-sans">{b.cta}</span>
                  </div>
                </div>
              )}
            </div>

    ...
    <div className="flex flex-col gap-3 font-sans antialiased">
      {!audit ? (
        ...
        <>
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-zinc-900 dark:text-white">
                  Funnel health: {overallSeverity === "none" ? "Stable" : `${overallSeverity.toUpperCase()} severity`}
                </span>
              </div>
              <span className="text-xs text-zinc-500 dark:text-zinc-400">
                {auditRunTypeLabel(audit.runType)} · {issues.length} metric{issues.length === 1 ? "" : "s"} evaluated
          ...
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-[#f8f7fa] dark:bg-zinc-950 p-3.5 transition-all">
            <div className="flex items-center justify-between">
              <button
                ...
                className="flex items-center gap-2 text-xs font-medium text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white cursor-pointer font-sans"
              >
                <ChevronDown size={14} className={cn("transition-transform duration-200", !isRunActivityOpen && "-rotate-90")} />
                <span>Show what happened during this run · 6 steps</span>
              </button>

              {/* Transparent Done pill */}
            ...
            {isRunActivityOpen && (
              <div className="mt-3 border-t border-zinc-200/80 dark:border-zinc-800/80 pt-3 space-y-2">
                <div className="flex items-center justify-between text-xs text-zinc-600 dark:text-zinc-400 px-1 py-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-900/50">
          ...
          {/* ----------------------------------------------------------------- */}
          {/* 3. OVERVIEW / CARDS VIEW                                          */}
          {/* ----------------------------------------------------------------- */}
          {mode === "calendar" && (
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-3 items-start font-sans">
          ...
          {/* ----------------------------------------------------------------- */}
          {/* 4. DENSE LIST VIEW WITH INLINE ACCORDION EXPANSION                 */}
          {/* ----------------------------------------------------------------- */}
          {mode === "list" && (
                        ...
                        >
                          <td colSpan={5} className="p-0">
                            <div
                              ...
                              className="flex items-center justify-between px-4 py-3 hover:bg-zinc-100/40 dark:hover:bg-zinc-900/40"
                            >
                              <div className="flex items-center gap-2 font-medium text-zinc-900 dark:text-white w-1/3">
                                <ChevronDown
                                  size={14}
                            ...
                            {/* Inline Diagnostic Assessment panel */}
                            {isExpanded && (
                              <div className="bg-white/50 dark:bg-zinc-900/50 p-4 border-t border-zinc-200/60 dark:border-zinc-800/60 space-y-2">
...
// ---------------------------------------------------------------------------
// ISSUE CARD WITH INLINE DIAGNOSTIC INSPECTION (NO DRAWER)
// ---------------------------------------------------------------------------
function IssueCard({
  issue,
  isExpanded,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Transparent pill style for NONE / Low severity badges */}
      ...
      {/* Render Diagnostic Info Directly Inline on the Card when expanded */}
      {isExpanded && (

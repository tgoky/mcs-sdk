      ...
      {config.productIdentity && (
        <div>
          <div className="flex items-center gap-2 pb-2 border-b border-zinc-200 dark:border-zinc-800 mb-2">
            ...
            <h3 className="text-[13px] font-bold uppercase tracking-wide text-zinc-700 dark:text-zinc-300">Product identity</h3>
          </div>
          <p className="text-sm text-zinc-800 dark:text-zinc-200 font-semibold">{config.productIdentity.name}</p>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">{config.productIdentity.valueProp}</p>
          <p className="text-[13px] font-mono text-zinc-400 dark:text-zinc-600 mt-1">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {config.icps.map((icp) => (
              <span
                key={icp.slug}
                className="text-[13px] font-mono px-2 py-1 rounded-md bg-zinc-100 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-800"
              >
      ...
      {config.voiceProfile && (
        <div>
          <div className="flex items-center gap-2 pb-2 border-b border-zinc-200 dark:border-zinc-800 mb-2">
            ...
            <h3 className="text-[13px] font-bold uppercase tracking-wide text-zinc-700 dark:text-zinc-300">Voice</h3>
          </div>
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            <span className="font-semibold text-zinc-800 dark:text-zinc-200">Tone:</span> {config.voiceProfile.tone}
          </p>
          <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1 italic">&ldquo;{config.voiceProfile.greeting}&rdquo;</p>
          <p className="text-sm text-zinc-600 dark:text-zinc-400 italic">&ldquo;{config.voiceProfile.signOff}&rdquo;</p>
          {config.subjectVariants.length > 0 && (
            <p className="text-[13px] font-mono text-zinc-400 dark:text-zinc-600 mt-1">{config.subjectVariants.length} subject variant(s)</p>
          )}
        </div>
      )}
      ...
      {config.leadSources.length > 0 && (
        <div>
          <div className="flex items-center gap-2 pb-2 border-b border-zinc-200 dark:border-zinc-800 mb-2">
            ...
            <h3 className="text-[13px] font-bold uppercase tracking-wide text-zinc-700 dark:text-zinc-300">Lead sources</h3>
          </div>
      ...
      {config.sendPlatform && (
        <div>
          <div className="flex items-center gap-2 pb-2 border-b border-zinc-200 dark:border-zinc-800 mb-2">
            ...
            <h3 className="text-[13px] font-bold uppercase tracking-wide text-zinc-700 dark:text-zinc-300">Send platform</h3>
          </div>
          <p className="text-sm text-zinc-800 dark:text-zinc-200 font-semibold">{config.sendPlatform.platform}</p>
          {Object.keys(config.campaignMap).length > 0 && (
            <div className="mt-1.5 space-y-0.5">
              {Object.entries(config.campaignMap).map(([icp, campaignId]) => (
                <p key={icp} className="text-[13px] font-mono text-zinc-500 dark:text-zinc-400">
                  {icp} → {campaignId}

  ...
  return (
    <div className="flex flex-col gap-3 font-sans antialiased">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-200 dark:border-zinc-800 pb-2">
        ...
        <p className="text-[13px] text-zinc-500 dark:text-zinc-500 pr-2">
      ...
      <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
        {filtered.map((f) => (
          <div key={f.id} className="py-3.5 first:pt-0">
            <div className="flex items-center justify-between gap-2 mb-1.5">
              <span className="text-[13px] font-mono font-bold uppercase tracking-wide text-zinc-600 dark:text-zinc-400">{f.engineId}</span>
              <div className="flex items-center gap-2">
                <FlaggedPill flagged={f.flagged} reason={f.flagReason} />
                <SentimentPill sentiment={f.sentiment} />
              </div>
            </div>
            <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 mb-1">{f.promptText}</p>
            <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed line-clamp-3">{f.responseText}</p>
            {f.flagReason && <p className="text-[13px] text-rose-600 dark:text-rose-400 mt-1.5">{f.flagReason}</p>}
          </div>
        ))}
        {filtered.length === 0 && (
          <p className="text-sm text-zinc-500 dark:text-zinc-500 italic text-center py-6">No findings match your search.</p>
        )}
      </div>
    </div>
  );
